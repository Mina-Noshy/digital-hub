...
public record DeleteModuleMasterCommand
    (long id, CancellationToken cancellationToken = default) : ICommand<ApiResponse>;


public class DeleteModuleMasterCommandHandler(IModuleMasterRepository _repository) : ICommandHandler<DeleteModuleMasterCommand, ApiResponse>
{

    public async Task<ApiResponse> Handle(DeleteModuleMasterCommand request, CancellationToken cancellationToken)
    {
