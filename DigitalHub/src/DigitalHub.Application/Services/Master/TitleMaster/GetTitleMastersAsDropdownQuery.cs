...
public record GetTitleMastersAsDropdownQuery
    (DropdownQueryParams queryParams, CancellationToken cancellationToken = default) : IQuery<ApiResponse>;

public class GetTitleMastersAsDropdownQueryHandler(ITitleMasterRepository _repository) : IQueryHandler<GetTitleMastersAsDropdownQuery, ApiResponse>
{
    public async Task<ApiResponse> Handle(GetTitleMastersAsDropdownQuery request, CancellationToken cancellationToken)
    {
